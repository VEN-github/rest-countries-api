...
export function header() {
  const template = `
    <header class="w-full bg-white shadow-md dark:bg-darkBlue">
      <div class="flex justify-between items-center container mx-auto py-8 px-4">
        <h1 class="font-extrabold text-fgDarkBlue md:text-2xl dark:text-white">Where in the World?</h1>
        <button id="darkModeBtn" class="flex justify-between items-center space-x-1.5 font-semibold">
          <ion-icon id="themeIcon" name="moon-outline" class="dark:text-white"></ion-icon>
          <span id="themeText" class="text-sm md:text-lg dark:text-white">Dark Mode</span>
        </button>
      </div>
    </header>
  `;
